...
for video_file in video_files:
    ...
    if total_frames >= 10: # Lowered threshold to 10 to ensure we don't drop all short videos during check
...
class video_dataset(Dataset):
    ...
    def __getitem__(self,idx):
        ...
        # Original code does: labels.loc[labels["file"] == temp_video]
        matching_rows = self.labels.loc[self.labels["file"] == temp_video]
        if len(matching_rows) == 0:
            # If not in labels, default to FAKE for safety, but log it
        ...
        while len(frames) < self.count:
            # pad if shorter than expected (should not happen if we check frame count, but just in case)
...
lr = 1e-5
num_epochs = 5  # Training for 5 epochs on the full processed dataset
