...
# Evaluate on 250 videos for much faster testing while still retaining a good trend representation
valid_videos = video_files[int(0.8*len(video_files)):int(0.8*len(video_files))+250]
...
print("\n| Evaluated Model Profile | Frame Sequence Length | Overall Accuracy |")
print("|---|---|---|")
...
with torch.no_grad():
    for f in frames_to_test:
        ...
        for i, (inputs, targets) in enumerate(valid_loader):
            ...
            running_correct += correct.float().sum().item()
            total_samples += targets.size(0)
            
            sys.stdout.write(f"\rEvaluating {f} frames... [{i}/{len(valid_loader)}]")
            sys.stdout.flush()
        
        acc = (running_correct / total_samples) * 100
        # Clear line
        sys.stdout.write("\r" + " " * 80 + "\r")
        sys.stdout.write(f"| model_v2_acc_{f}_frames | {f} Frames | {acc:.2f}% |\n")
        sys.stdout.flush()
        results.append(f"| model_v2_acc_{f}_frames | {f} Frames | {acc:.2f}% |")

with open('new_accuracy_table.md', 'w') as f:
    f.write("| Evaluated Model Profile | Frame Sequence Length | Overall Accuracy |\n")
    f.write("|---|---|---|\n")
    for r in results:
         f.write(r + "\n")
print("\nResults successfully saved to new_accuracy_table.md")
