...
print("Loading trained model weights for confusion matrix...")
model = Model(2)
...
model.eval()
val_data = video_dataset(valid_videos,labels,sequence_length=seq_length,transform=train_transforms)
...
with torch.no_grad():
    for i, (inputs, targets) in enumerate(valid_loader):
        ...
        sys.stdout.write(f"\rEvaluating for Confusion Matrix... [{i+1}/{len(valid_loader)}]")
        sys.stdout.flush()
...
plt.title(f'Deepfake Model Confusion Matrix (40 Frames)')
plt.tight_layout()

# Save into the Agent artifact brain folder directly so we can embed it in markdown easily 
dest_path = '/home/aryan/.gemini/antigravity/brain/04e66fdd-7e9d-4bb8-a2ff-c48a23721168/confusion_matrix.png'
plt.savefig(dest_path)
print(f"\nConfusion matrix successfully saved as {dest_path}")
